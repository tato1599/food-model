...
# ==========================================
# REDIMENSIONAMIENTO A 224x224 (CALIDAD MAXIMA)
# ==========================================
...
print("\n--- Verificando redimensionamiento a 224x224 ---")
if not os.path.exists(OUTPUT_ROOT) or len(os.listdir(OUTPUT_ROOT)) < 30:
    print("Creando imagenes 224x224 desde originales...")
    ...
    print(f"Redimensionamiento 224x224 completado en {time.time() - start_resize:.2f}s")
else:
    print("Imagenes 224x224 ya existen, saltando...")

# ==========================================
# DATASETS Y DATALOADERS (MEJORADO AL MAXIMO)
# ==========================================
print("\n--- Cargando datasets 224x224 ---")

# Media y std del dataset original (aproximadamente similar para 224)
...
# Batch size 32 para 224x224 (mayor uso de VRAM)
...
# ==========================================
# MODELO: RESNET50 PRE-ENTRENADA (FINE-TUNING PROFUNDO)
# ==========================================
class FoodResNet50(nn.Module):
    def __init__(self, num_classes=34):
        ...
        # Congelar las primeras capas (layer1 y layer2)
        for param in self.backbone.parameters():
            param.requires_grad = False
        
        # Descongelar layer3, layer4 y fc para fine-tuning profundo
...
# ==========================================
# ENTRENAMIENTO CON EARLY STOPPING Y LABEL SMOOTHING
# ==========================================
...
def train(model, train_loader, val_loader, epochs=40, lr=5e-4, patience=7, weight_decay=1e-4):
    model.to(DEVICE)
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("MODELO FINAL: ResNet50 + 224x224 + Fine-tuning Profundo")
    print("="*60)
    ...
    print(f"Ratio entrenable: {trainable_params/total_params*100:.1f}%")
    
    best_val_acc, history = train(
        model, train_loader, val_loader, 
        epochs=40, lr=5e-4, patience=7, weight_decay=1e-4
    )
    ...
    print(f"\nTiempo total de ejecucion: {total_time:.2f}s ({total_time/60:.1f} min)")
    ...
    print("\nGuardado como checkpoint completo (incluye clases e historia)")
